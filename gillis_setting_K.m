...
list_eps = [0.01:0.01:0.09 0.1:0.1:1];
% list_SNR = [5:1:40];
SNR = [];

t0 = tic;
for exp_ind=1:numel(list_eps)
    fprintf('%d/%d - Elapsed time: %f \n', exp_ind, numel(list_eps), toc(t0));
    ...
    for trial_ind=1:5
        ...
        % noise_energy = sqrt(norm(Y, 'fro')^2/exp(list_SNR(exp_ind)/10));
        noise = original_noise/norm(original_noise, 'fro')*list_eps(exp_ind);
...
save('./results/gillis_setting.mat')
load('./results/gillis_setting.mat')
% figure();
...
saveas(gcf, './results/gillis_setting/RAE.eps', 'epsc')


figure();
semilogx(list_eps, mean(RAE_spa, 2), '-x', 'DisplayName',  '\texttt{SPA}')
hold on
semilogx(list_eps, mean(RAE_fw, 2), '-s', 'DisplayName',  '\texttt{MERIT}')
semilogx(list_eps, mean(RAE_fg, 2), '-o', 'DisplayName',  '\texttt{FastGradient}')
xlabel('Noise level')
ylabel('\texttt{Relative approximation error}', 'Interpreter',  'latex')
legend('Location',  'NorthWest')
...
xlabel('SNR')
legend
saveas(gcf, './results/gillis_setting/MRSA.eps', 'epsc')


tmp = max([max(MRSA_spa(:)) max(MRSA_fw(:)) max(MRSA_fg(:))]);
figure();
semilogx(list_eps, mean(MRSA_spa, 2)/tmp*100, '-x', 'DisplayName',  '\texttt{SPA}')
hold on
semilogx(list_eps, mean(MRSA_fw, 2)/tmp*100, '-s', 'DisplayName',  '\texttt{MERIT}')
semilogx(list_eps, mean(MRSA_fg, 2)/tmp*100, '-o', 'DisplayName',  '\texttt{FastGradient}')
ylabel('\texttt{MRSA}', 'Interpreter',  'latex')
xlabel('Noise level')
legend
...
xlabel('SNR')
legend('Location',  'NorthWest')
saveas(gcf, './results/gillis_setting/overlapping.eps', 'epsc')
...
xlabel('SNR')
legend('Location',  'NorthWest')
saveas(gcf, './results/gillis_setting/success_rate.eps', 'epsc')
